...
def sm3d(centre, map_folder, distance=15, load=150, biomes=(), planets=(0, 50), opacity=30, search_mode='ball_point',
         path_id='None', save=False, hide_ids=False):
    ...
    for s in star_index:
        ...
        if hide_ids and s == sd[s]['name']:
            label = sd[s]['biome_mode']
        else:
            label = sd[s]['name'] + '<br>' + sd[s]['biome_mode']
        id_names.append(s)
        for t in sd[s]['tags']:
            label += '<br>' + t
        labels.append(label)
...
def edit_pane(node, map_folder):
    ...
    pane = [dbc.Card(dbc.Button(children=sd[node]['name'], id={'edit-button': 0}, color='primary'),
                     id='edit-pane' + str(0),
                     body=True, color=ck[sd[node]['biome_mode']],
                     style={'border': '1px solid #00001a'}),
            dbc.Tooltip(sd[node]['biome_mode'], target='edit-pane' + str(0), placement='left'),
            dbc.Tooltip(node, target='edit-pane' + str(0), placement='right'),
            html.Hr()]
    n = 1
    for planet in sd[node]['planets']:
        pane.append(dbc.Card(dbc.Button(children=planet['name'], id={'edit-button': n}, color='primary'),
                             id='edit-pane'+str(n),
                             body=True,
    ...
    modal = dbc.Modal(
        [
            dbc.ModalHeader("Edit data", id='modal_title', style=dark_style),
            dbc.ModalBody(
                [
                    ...
                    dcc.Dropdown(
                        id='modal_input_tags',
                        options=[{'label': i, 'value': i} for i in default.tags],
                        value=(),
                        multi=True,
                        placeholder='tag',
                        style=dark_style),
    ...
    return dbc.Card(pane, body=True, id='edit-panes', color=ck[sd[node]['biome_mode']])
...
pane_cards = edit_pane(parameters['centre'], parameters['map_folder'])
print('pane_cards')
...
tabs = html.Div([dbc.Tabs([
    ...
    html.Hr(),
    dbc.Checklist(
        options=[{"label": "show axes", "value": 1}, {"label": "save map", "value": 2}, {"label": "hide ids", "value": 3}],
        value=[],
        id="checklist-input",
        style={'backgroundColor': default.plotly_colours['background'],
               'border': default.plotly_colours['background'],
...
def search_update(figure, clickData, button, biomes, planet_slider, range_slider, chunk_slider,
    ...
    # print(select_button)
    # print(clickData)
    if 3 in checklist_input:
        parameters['hide_ids'] = True
    else:
        parameters['hide_ids'] = False
    ...
    if button != presses['search_button']:
        ...
        # print(select_id)
        if select_id == '':
            select_id = parameters['centre']
        else:
            parameters['centre'] = select_id
        # print(parameters['centre'])
...
def define_modal(node, button):
    import orjson
    presses['modal_button'] = button
    chunk = node.split('-')[0]
    with open(parameters['map_folder']+chunk+'.json', 'rb') as f:
        sd = orjson.loads(f.read())
    if button == 0:
        return True, sd[node]['name'], sd[node]['biome_mode'], dash.no_update, sd[node]['tags']
    else:
        n = 1
        for planet in sd[node]['planets']:
            if n == button:
                return True, planet['name'], planet['biome'], dash.no_update, sd[node]['tags']
            else:
                n += 1


@app.callback(
    Output({'edit-button': ALL}, 'n_clicks'),
    Output('test_modal', 'is_open'),
    Output('modal_input_name', 'value'),
    Output('modal_input_biome', 'value'),
    Output('select-id', 'n_clicks'),
    Output('modal_input_tags', 'value'),
    Output('modal_ok_button', 'n_clicks'),
    Input({'edit-button': ALL}, 'n_clicks'),
    Input('modal_ok_button', 'n_clicks'),
    State('modal_input_name', 'value'),
    State('modal_input_biome', 'value'),
    State('modal_input_tags', 'value')
)
def show_modal(buttons, ok, name, biome, tags):
    """Show modal for adding a label."""
    import orjson
    # if any button hit, use the current seed and the button hit to define the modal
    # show taht modal
    button = [i for i in range(len(buttons)) if buttons[i]]
    print('show modal')
    print(buttons)
    print(button)
    if len(button) > 0:
        return [None for _ in buttons], *define_modal(parameters['centre'], button[0]), 0
    ...
    if ok:
        print('modal button')
        print(presses['modal_button'])
        if presses['modal_button'] == 0:
            sd[c]['name'] = name
            sd[c]['biome_mode'] = biome
            sd[c]['tags'] = tags
        else:
            sd[c]['planets'][presses['modal_button']-1]['name'] = name
            sd[c]['planets'][presses['modal_button']-1]['biome'] = biome
        with open(parameters['map_folder']+chunk+'.json', 'wb') as f:
            print('wrote it')
            f.write(orjson.dumps(sd))
        return [None for _ in buttons],\
               False,\
               parameters['centre'],\
               dash.no_update,\
